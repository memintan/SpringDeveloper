...
--write a quert that returns the first name and email of females that work in the tools department
...
--Write a query that returns the first anme and hire date of those employees who earn more than 165.000
...
--Write a query that returns students based on the following criteria;
    --The student mustn't older than 20 if their student_no is either between 3-5
    --or their student_no is 7. Your query should also return students older than age 20 but in that case they must
...
SELECT TRIM('    Hello Buddy                 ');

SELECT LENGTH(TRIM('     Hey Buddy     '));
...
SELECT MIN(salary), MAX(salary),ROUND(AVG(salary)), COUNT(*) total_number_of_employees, department
FROM employees
GROUP BY department;
